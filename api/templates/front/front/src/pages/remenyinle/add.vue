...
<div :style='{"width":"100%","padding":"20px 10%","margin":"20px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="语言" prop="yuyan">
            <el-input v-model="ruleForm.yuyan" 
                placeholder="语言" clearable :disabled=" false  ||ro.yuyan"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="专辑" prop="zhuanji">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="发行方" prop="faxingfang">
            <el-input v-model="ruleForm.faxingfang" 
                placeholder="发行方" clearable :disabled=" false  ||ro.faxingfang"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="歌名" prop="songname">
            <el-input v-model="ruleForm.songname" 
                placeholder="歌名" clearable :disabled=" false  ||ro.songname"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="音乐文件" prop="songfile">
            <file-upload
            tip="点击上传音乐文件"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="歌手" prop="singer">
            <el-input v-model="ruleForm.singer" 
                placeholder="歌手" clearable :disabled=" false  ||ro.singer"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="歌词" prop="geci">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="歌词"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="歌曲简介" prop="gequjianjie">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`remenyinle/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('remenyinle/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`remenyinle/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
