...
@main_bp.route("/python1p21sd8o/cal/<tableName>/<columnName>", methods=['GET'])
def python1p21sd8o_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/python1p21sd8o/file/download", methods=['GET'])
def python1p21sd8o_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/python1p21sd8o/file/upload", methods=['POST'])
def python1p21sd8o_file_upload():
    '''
    '''
...
@main_bp.route("/python1p21sd8o/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def python1p21sd8o_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python1p21sd8o/follow/<tableName>/<columnName>", methods=['GET'])
def python1p21sd8o_follow(tableName, columnName):
    '''
...
@main_bp.route("/python1p21sd8o/location", methods=['GET'])
def python1p21sd8o_location():
    '''
...
@main_bp.route("/python1p21sd8o/matchFace", methods=['GET'])
def python1p21sd8o_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/python1p21sd8o/option/<tableName>/<columnName>", methods=['GET'])
def python1p21sd8o_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python1p21sd8o/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def python1p21sd8o_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python1p21sd8o/sh/<tableName>", methods=['POST'])
def python1p21sd8o_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("python1p21sd8o_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/python1p21sd8o/upload/<fileName>", methods=['GET'])
def python1p21sd8o_upload(fileName):
    '''
    上传接口
    '''
    if request.method == 'GET':
...
@main_bp.route("/python1p21sd8o/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/python1p21sd8o/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
# 爬取数据
@main_bp.route("/python1p21sd8o/spider/<tableName>", methods=['GET'])
def schema_spider(tableName):
    msg = {"code": normal_code, "msg": "success", "data": []}

    # Linux
    cmd = "cd /yykj/python/9999/spider0ebqh2vm && scrapy crawl "+tableName+"Spider -a databaseName=python1p21sd8o"
    # Windows
    # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=python1p21sd8o"
    os.system(cmd)

    return jsonify(msg)

# 数据备份
@main_bp.route("/python1p21sd8o/mysqldump", methods=['GET'])
def schema_mysqldump():
    if request.method == 'GET':
        filename = "mysql.dmp"
        filepath = '{}/api/templates/front'.format(os.getcwd())

        cmd = "/usr/bin/mysqldump -h127.0.0.1 -uroot -P3306 -p123456 python1p21sd8o > " + filepath + "/" + filename
        os.system(cmd)
